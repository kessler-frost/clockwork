...
full_stack.children["database-cluster"].children["postgres-replica"].connect(
    full_stack.children["database-cluster"].children["postgres-primary"]
)

# API needs primary database (for writes)
full_stack.children["application-tier"].children["api-service"].connect(
    full_stack.children["database-cluster"].children["postgres-primary"]
)

# Worker needs primary database (for writes)
full_stack.children["application-tier"].children["worker-service"].connect(
    full_stack.children["database-cluster"].children["postgres-primary"]
)

# Frontend needs replica (for reads) and API
full_stack.children["application-tier"].children["web-frontend"].connect(
    full_stack.children["database-cluster"].children["postgres-replica"]
)
full_stack.children["application-tier"].children["web-frontend"].connect(
