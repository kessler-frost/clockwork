"""
Resource Completer - Intelligence-powered resource completion using PydanticAI structured outputs.

This module uses schema-native completion where Pydantic Field descriptions define
the completion task. No prompts needed - the schema IS the prompt!
"""
...
class ResourceCompleter:
    """Completes partial resources using intelligent completion via PydanticAI structured outputs."""
    ...
    def _get_tools_for_resource(self, resource: Any) -> list[Any]:
        """
        Get tools for a resource - prioritize user-provided tools, then use ToolSelector.

        Args:
            resource: Resource object to get tools for

        Returns:
            List of tools to use for completion
        """
        if resource.tools is not None and len(resource.tools) > 0:
            logger.debug(f"Using {len(resource.tools)} user-provided tools")
            return resource.tools

        if self.enable_tool_selection and self.tool_selector:
    ...
    def _create_agent(
        self, model: OpenAIChatModel, tools: list[Any], output_type: type
    ) -> Agent:
        """
        Create a PydanticAI agent with validation.

        Args:
            model: OpenAI-compatible model instance
            tools: List of tools to provide to the agent
            output_type: Type class for structured output

        Returns:
            Configured Agent instance with output validator
        """
        settings = get_settings()
        agent = Agent(
            model,
    ...
    async def complete(
        ...
        already_complete = [r for r in resources if not r.needs_completion()]

        for r in already_complete:
            logger.info(f"Resource already complete: {r.name}")

        for r in to_complete:
            logger.info(f"Queuing resource for completion: {r.name}")

        # Complete all resources in parallel
        completed = []
        if to_complete:
            completed = list(
            ...
            for r in completed:
                logger.info(f"Completed resource: {r.name}")

        return already_complete + completed
    ...
    async def _complete_resource(self, resource: Any) -> Any:
        ...
        return await self._run_completion(resource, user_message)

    def _merge_resources(
        self, user_resource: Any, completed_resource: Any
    ) -> Any:
        """
        ...
        for field_name in user_data:
            ...
            # BUT: if user value is None and completed has a value, use completed
        ...
        # Preserve connections from user resource (not part of completion)
        ...
        # Preserve assertions from user resource (not part of completion)
        # Assertions should always come from the user, never generated by completion
    ...
    async def _complete_composite(
        ...
        This enables intelligent completion to:
